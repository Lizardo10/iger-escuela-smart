...
export const ResponsiveLogo: React.FC<ResponsiveLogoProps> = ({ 
  ...
  return (
    <div className={`flex items-center space-x-2 sm:space-x-3 ${className}`}>
      {/* Logo SVG responsivo */}
      <div className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 lg:w-16 lg:h-16 flex-shrink-0 logo-float logo-hover">
        <svg
          viewBox="0 0 100 100"
      ...
      {/* Texto del logo responsivo */}
      {showText && (
        <div className="flex flex-col">
          <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-blue-600 animate-float">
            IGER
          </h1>
          <p className="text-blue-500 text-xs sm:text-sm font-medium hidden sm:block">
            Escuela Smart
          </p>
        </div>
      )}
